...
1. Para instalar e utilizar esse projeto o processo é bem simples, basta clonar um projeto  utilizando o comando:

```sh
git clone 'esta url'
```

2. Depois do projeto clonado é necessário rodar o seguinte comando para instalar as dependencias:
```sh
yarn 
```

3. Com as bibliotecas instaladas, de modo a executar o aplicativo, é necessário conectar um emulador android ou um telefone celular que permita testes do aplicativo e rodar os seguintes comandos:
 a. ```sh yarn start --reset-cache ```
 Em outra janela aberta no projeto, execute:
 b. ```sh yarn android ```
